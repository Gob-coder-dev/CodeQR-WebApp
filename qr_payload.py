...
from qr_service import QRCodeRequestError, parse_bool
...
def build_qr_payload(payload) -> str:
    ...
    if qr_type == "phone":
        return f"tel:{require_value(payload, 'phone_number', 'Please enter a phone number.')}"
...
def build_sms_payload(payload) -> str:
    ...
    return f"sms:{phone_number}?body={quote(message)}"
...
def build_contact_payload(payload) -> str:
    full_name = require_value(payload, "contact_name", "Please enter a contact name.")
    lines = [
        "BEGIN:VCARD",
        "VERSION:3.0",
